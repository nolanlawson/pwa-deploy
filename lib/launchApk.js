...
function execAdb (specificDevice, commands) {
  return adb().then(function (adbPath) {
    var promise = Promise.resolve();
    commands.forEach(function (cmd) {
      promise = promise.then(function () {
        var fullCmd = (specificDevice ? ('-s ' + specificDevice + ' ') : '') + cmd;
...
function launchApk (dir, specificDevice, port) {
  return stat(dir).then(function (stats) {
    if (!stats.isDirectory()) {
      throw new Error(dir + ' is not a directory. ' +
        'Please enter a valid directory path.');
    }
    return execAdb(specificDevice, [
